...
@media (max-width: 768px) {
  ...
  .weekly-schedule {
    grid-template-columns: repeat(3, 1fr);
      ...
      /* ======================== LEADS PREMIUM SECTION ======================== */
.leads-premium-container {
  background: linear-gradient(135deg, #ffffff 0%, #f8fafc 100%);
  border-radius: 1.5rem;
  padding: 1.25rem;
  margin-top: 1rem;
  border: 1px solid rgba(116, 48, 226, 0.12);
  box-shadow: 0 20px 35px -12px rgba(0, 0, 0, 0.06);
}

.leads-premium-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: 1.5rem;
...
.leads-icon-badge {
  ...
  color: white;
  font-size: 1.3rem;
  box-shadow: 0 8px 20px rgba(116, 48, 226, 0.25);
}
...
.metric-card-premium.conversion {
  grid-column: span 2;
  display: flex;
  align-items: center;
  gap: 1rem;
...
.metric-card-premium.conversion .metric-info {
  flex: 1;
  color: white;
}

.metric-card-premium.conversion .metric-info i {
  color: #10b981;
  margin-right: 0.4rem;
}

.metric-card-premium.conversion .metric-info span {
  font-size: 0.7rem;
  opacity: 0.8;
  display: block;
}

.metric-card-premium.conversion .metric-info strong {
  font-size: 0.9rem;
  display: block;
  margin-top: 0.2rem;
}
...
.metric-card-premium.hot-leads .metric-icon-premium {
  background: #fef2f2;
  color: #ef4444;
}

.metric-card-premium.engaged .metric-icon-premium {
  background: #eff6ff;
  color: #3b82f6;
}

.metric-card-premium.lost .metric-icon-premium {
  background: #ecfdf5;
  color: #10b981;
}
...
@media (max-width: 768px) {
  ...
  const [selectedSocialPlatform, setSelectedSocialPlatform] = useState('facebook')
...
async function createAppointmentAgent() {
  ...
  const sendVideoToSocialAI = (job) => {
    ...
    setGeneratedSocialContent(prev => {
  ...
  async function publishSocialNow() {
    ...
    try {
      ...
      setSocialActionText('Conectando con ' + (selectedSocialPlatform === 'tiktok' ? 'TikTok' : selectedSocialPlatform === 'instagram' ? 'Instagram' : 'Facebook') + '...')
      ...
      showNotice(`Publicación enviada a ${selectedSocialPlatform}`)
      await loadSocialPosts()
      await loadSocialLogs()
    } catch (err) {
      showNotice(err.message || 'Error publicando en la red social')
    } finally {
      setTimeout(() => {
  ...
  async function scheduleSocialPost() {
    ...
    try {
      ...
      showNotice(`Publicación programada para ${selectedSocialPlatform}`)
      await loadSocialPosts()
      await loadSocialLogs()
    } catch (err) {
  ...
  async function generateSocial() {
    ...
    try {
      ...
      setSocialContent(generated)
      setGeneratedSocialContent(generated)
  ...
  async function publishMulti() {
    ...
    if (!selectedPlatforms.length) {
      showNotice('Selecciona Facebook, Instagram o TikTok')
      return
    }
    ...
    try {
      const res = await api(`/api/social/publish-multi?client_id=${selectedClientId}`, {
        ...
        image_url: imageURL,
        video_url: socialMediaType === 'video' ? selectedSocialVideoURL : '',
      ...
      if (selectedPlatforms.includes('tiktok') && tk && tk.ok === false) {
        showNotice(`TikTok falló: ${tk.error || 'error desconocido'}`)
        return
      }
  ...
  async function loadSocialCredentials() {
    try {
      ...
      setSocialCredential(prev => ({
        ...
        tiktok_open_id: res.tiktok_open_id || '',
        tiktok_connected: res.tiktok_connected || false
      }))
          ...
          <div className="loading-card">
            ...
            <div className="loading-subtitle">
              {socialImageLoading
                ...
                : (socialActionText || 'Conectando con la red social y procesando...')}
            </div>
            <div className="loading-bar">
              <div
      ...
      <main className="main-pane">
        ...
        {tab === 'dashboard' && (
          <section className="stack gap-lg">
           ...
           {/* ======================== LEADS POR ETAPA - PREMIUM ======================== */}
<div className="leads-premium-container">
  <div className="leads-premium-header">
    <div className="leads-header-left">
      <div className="leads-icon-badge">
        <i className="fas fa-chart-line"></i>
      </div>
      <div>
        <h3 className="leads-title">Pipeline de conversión</h3>
        <p className="leads-subtitle">Distribución de leads por etapa del embudo</p>
      </div>
    </div>
    <div className="leads-header-right">
      <div className="total-leads-badge">
        <span className="total-label">Total leads</span>
        <span className="total-number">{leads.filter(l => l.bot_id === selectedBot.id).length}</span>
      </div>
    </div>
  </div>

  <div className="leads-premium-grid">
    {/* GRÁFICA DE BARRAS HORIZONTALES PREMIUM */}
    <div className="leads-chart-premium">
      <div className="chart-stage-list">
        {botLeadsByStage.stages.map((stage, idx) => {
          ...
          const stageConfig = {
            new: { icon: 'fas fa-star', color: '#3b82f6', bg: '#eff6ff', label: 'Nuevos', desc: 'Primer contacto' },
            qualified: { icon: 'fas fa-check-circle', color: '#8b5cf6', bg: '#f5f3ff', label: 'Calificados', desc: 'Interés validado' },
            interested: { icon: 'fas fa-fire', color: '#f59e0b', bg: '#fffbeb', label: 'Interesados', desc: 'Alta intención' },
            hot: { icon: 'fas fa-bolt', color: '#ef4444', bg: '#fef2f2', label: 'Hot leads', desc: 'Listos para cerrar' },
            closed: { icon: 'fas fa-trophy', color: '#10b981', bg: '#ecfdf5', label: 'Cerrados', desc: 'Venta concretada' }
          }
          ...
          return (
            <div key={stage} className="stage-row">
              <div className="stage-info">
                ...
                <div className="stage-details">
                  <div className="stage-name">
                    {config.label}
                    <span className="stage-badge" style={{ background: config.color }}>
                      {stage}
                    </span>
                  </div>
                  <div className="stage-desc">{config.desc}</div>
                </div>
                <div className="stage-count">
                  <span className="count-number">{count}</span>
                  <span className="count-unit">leads</span>
                </div>
              </div>
              <div className="progress-bar-premium">
                ...
                >
                  <span className="progress-percentage">{Math.round((count / (botLeadsByStage.counts.reduce((a,b) => a+b, 0) || 1)) * 100)}%</span>
                </div>
              </div>
            </div>
          )
    ...
    {/* TARJETA DE MÉTRICAS Y KPIS */}
    <div className="leads-metrics-premium">
      <div className="metrics-grid-premium">
        <div className="metric-card-premium conversion">
          <div className="metric-ring">
            <svg viewBox="0 0 100 100">
              ...
              <circle 
                ...
                strokeWidth="8"
                strokeDasharray={`${(botLeadsByStage.counts[4] / (botLeadsByStage.counts.reduce((a,b) => a+b, 0) || 1)) * 264} 264`}
                strokeLinecap="round"
            ...
            <div className="ring-inner">
              <span className="ring-value">
                {Math.round((botLeadsByStage.counts[4] / (botLeadsByStage.counts.reduce((a,b) => a+b, 0) || 1)) * 100)}%
              </span>
              <span className="ring-label">Tasa cierre</span>
            </div>
          </div>
          <div className="metric-info">
            <i className="fas fa-chart-simple"></i>
            <span>Conversión general</span>
            <strong>{botLeadsByStage.counts[4]} de {botLeadsByStage.counts.reduce((a,b) => a+b, 0)} leads</strong>
          </div>
        </div>

        <div className="metric-card-premium hot-leads">
          <div className="metric-icon-premium">
            <i className="fas fa-bell"></i>
            <div className="pulse-dot"></div>
          </div>
          <div className="metric-data">
            <span className="metric-label">Hot leads</span>
            <strong className="metric-value">{botLeadsByStage.counts[3] || 0}</strong>
            <span className="metric-trend">Listos para cerrar</span>
          </div>
        </div>

        <div className="metric-card-premium engaged">
          <div className="metric-icon-premium">
            <i className="fas fa-comments"></i>
          </div>
          <div className="metric-data">
            <span className="metric-label">En conversación</span>
            <strong className="metric-value">
              {botLeadsByStage.counts.slice(1, 4).reduce((a,b) => a+b, 0) || 0}
            </strong>
            <span className="metric-trend">Activos en funnel</span>
          </div>
        </div>

        <div className="metric-card-premium lost">
          <div className="metric-icon-premium">
            <i className="fas fa-flag-checkered"></i>
          </div>
          <div className="metric-data">
            <span className="metric-label">Cerrados</span>
            <strong className="metric-value">{botLeadsByStage.counts[4] || 0}</strong>
            <span className="metric-trend">Ventas concretadas</span>
          </div>
        </div>
      </div>

      {/* TASA DE CONVERSIÓN POR ETAPA */}
      <div className="conversion-funnel-premium">
        ...
        <div className="funnel-steps">
          {[
            { stage: 'new', label: 'Nuevos', count: botLeadsByStage.counts[0] || 0 },
            { stage: 'qualified', label: 'Calificados', count: botLeadsByStage.counts[1] || 0 },
            { stage: 'interested', label: 'Interesados', count: botLeadsByStage.counts[2] || 0 },
            { stage: 'hot', label: 'Hot', count: botLeadsByStage.counts[3] || 0 },
            { stage: 'closed', label: 'Cerrados', count: botLeadsByStage.counts[4] || 0 }
          ].map((step, idx, arr) => {
                ...
                <div className="funnel-step-left">
                  <div className="step-bullet" style={{ 
                    background: `linear-gradient(135deg, 
                      ${step.stage === 'new' ? '#3b82f6' : 
                        step.stage === 'qualified' ? '#8b5cf6' :
                        step.stage === 'interested' ? '#f59e0b' :
                        step.stage === 'hot' ? '#ef4444' : '#10b981'} 0%, 
                      ${step.stage === 'new' ? '#2563eb' : 
                        step.stage === 'qualified' ? '#6d28d9' :
                        step.stage === 'interested' ? '#d97706' :
                        step.stage === 'hot' ? '#dc2626' : '#059669'} 100%)`
                  }}>
                    {idx + 1}
                  </div>
                  <div>
                    <div className="step-label">{step.label}</div>
                    <div className="step-count">{step.count} leads</div>
                  </div>
                </div>
